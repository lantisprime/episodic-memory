...
if (argv.includes('--help') || argv.includes('-h')) {
  console.log(JSON.stringify({ status: 'help', script: 'em-consolidate.mjs', usage: 'node em-consolidate.mjs [--scope local|global] [--min-sim <0..1>] [--min-cluster <n>] [--category <cat>] [--project <name>] [--include-pinned] [--apply] [--confirm] — fold near-duplicate episodes into digest episodes (dry-run by default) | --fold-superseded [--min-chain <n>] [--dry-run] [--all-projects] — archive non-terminal members of long supersedes-chains (reversible; terminal untouched; history walk still resolves the chain). --all-projects (fold mode only, mutually exclusive with --scope) folds every consumer-registry store; a real multi-store run requires --confirm; per-store R6 protection unions cwd-local + global + all registered stores | --clerk [--scope local|global] [--window-ms <ms>] [--zero-conversion-runs <n>] — R9b lexical clerk report (read-only over episodes and index; consumes activation-log.jsonl via rotate-and-consume; the R6 conversion metric is a binary lower bound folded into the run-record; the report surfaces lessons with >= --zero-conversion-runs (default 3) consecutive zero-conversion runs as reword/demote/suppress candidates) | --clerk --apply --confirm [--scope local|global] [--reject-all] [--reject-member <id>]... [--lock-timeout <s>] [--window-ms <ms>] — R9b clerk apply (fails closed without --confirm; holds the store clerk-apply.lock for the whole apply, --lock-timeout <s> bounds acquisition, default 30; --reject-all rejects every proposed cluster, repeatable --reject-member <id> rejects any cluster containing that member; writes exactly one clerk-run run-record) | --clerk --enrich [--apply] [--confirm] [--reject-member <id>]... — R9c lexical enrichment backfill (proposes triggers/applies_to_project/applies_to_tool from the episode\'s OWN project and tool provenance fields ONLY, never widened; per-item confirm; apply writes via em-revise AS-IS and a run-record under the same lock)' }))
  process.exit(0)
}

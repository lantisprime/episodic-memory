...
- **R7d Aggregator dependency.** If implemented agentically, it consumes the R9d prompt-as-data convention — which requires the #531 installer/deploy-audit blindness fix to land first. The zero-dep lexical form has no such dependency and MAY ship first.

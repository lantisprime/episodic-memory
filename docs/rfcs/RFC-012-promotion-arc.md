...
- **R2d Assisted apply + run-record.** `--apply` gains a per-recurrence confirm gate mirroring the clerk's `--apply --confirm` (closing the B-2 gap: today `scripts/em-promote.mjs:320-328` writes each promotion with no confirmation), and each apply run writes a run-record episode mirroring the clerk's (`record_type: promote-run` class) — operational metadata per Principle 7, exempt from per-item confirmation, giving R3c its delta source and closing the idempotency-observability gap.
...
- **R4a Capture nudge classes.** The activation adapter MAY emit, as part of its existing bounded `additionalContext`, a one-line write-side nudge when a registered nudge condition matches. Nudge conditions are schema-backed matcher definitions shipped as adapter data (B-5) — proposed initial set: a decision-shaped prompt (a registered phrase-class matcher, same machinery as the RFC-009 R2 trigger index's `phrase` kind) and a work-milestone tool event (merge/push class). Matched at the same three events the RFC-009 R3/R4 adapter already binds — no new hooks, no new events.
- **R4b Confirm-before-store holds.** A nudge only ever suggests `em-store` / `em-capture` invocations; it writes nothing itself (B-2).
- **R4c Suppression.** Nudges respect the existing suppression mechanism and the RFC-009 R3 caps; a nudge line competes inside `max_tokens`, never extends it. Per-session nudge budget of 1: nudge emissions append to the existing activation-telemetry log (`activation-log.jsonl`, additive record shape registered as data) keyed by session, and the adapter checks the current session's entries before emitting; where no session key is derivable the budget degrades to per-event best effort. No new store is created — the log already exists.

This is a capture *nudge*, not capture itself: it prompts an `em-store`/`em-capture` invocation mid-session but performs no write (B-2), and the SessionEnd draft extraction stays the only automatic capture path (`scripts/em-session-end-prompt.mjs:195-218`) — R4 mitigates the momentum gap rather than closing it (OQ-9). CORAL's reflection heartbeat translated to lifecycle events: the trigger is a matched prompt/tool event, never a clock.
...
**Reviewer:** <!-- pending — codex gpt-5.6-sol (high), independent parallel round -->
**Date:** <!-- pending -->
**Findings:** <!-- pending -->

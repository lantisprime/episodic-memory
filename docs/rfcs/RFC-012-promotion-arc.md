...
- **R4a Nudge lessons.** Ship a small curated set of global `lesson` episodes (data, explicitly opt-in at install — P3) whose RFC-009 R2 triggers match decision-language phrase classes and milestone tool events (merge/push class), and whose one-line summaries prompt an `em-store`/`em-capture` invocation. The plane surfaces them exactly like any lesson: same matcher machinery, same `max_matches`/`max_tokens` caps, same per-episode-id suppression — no new hooks, no new events, no new read surface, no session state. RFC-009's event-plane read set stays untouched (trigger-index.json + category-index.json only, `docs/rfcs/RFC-009-lesson-activation.md:43`).
...
- RFC-010 (Versioned Central Engine) — adjacent, no dependency: R7's draft guards activate only through the per-project enforcement install path; if RFC-010's central-engine model ships, R7 proposals target its versioned payloads.
...
| OQ-10 | R9a drift: what audit bounds a clerk from proposing scaffold revisions that weaken its own evidence-grounding across iterations (e.g., a guardrail that evidence-citation density must not decrease across consecutive prompt revisions)? | Charlton Ho | open |

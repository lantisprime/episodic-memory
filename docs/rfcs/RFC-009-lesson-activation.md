...
- **Event plane (hooks).** Per-project, opt-in, per the harness's declared capability tier (R3 tier table, P5; manifest in the RFC-008 R8 typed registry). Fires on UserPromptSubmit and tool events, and is MECHANICAL and cheap by contract: reads ONLY derived-index data and NEVER episode bodies, never invokes a model, always exits 0 with no decision field. "Derived-index data" is defined precisely as the store's derived-index family — `index.jsonl`, `tags.json`, `trigger-index.json`, `category-index.json` — every member mechanically rebuildable from episode files (P1: episodes are the only source of truth; everything in the family is regenerable metadata). The load-bearing boundary is bodies-vs-metadata plus no-model, not which derived file: a read-only `em-recall --no-track` consuming index rows is event-plane-legal; opening an episode `.md` is not. Its writes are exactly two, both mechanical: the append-only, size-bounded telemetry line of R6 (fire-and-forget, never blocking, never fatal), and the **derived-index refresh carve-out** — when a consumer read finds a derived index stale (R2 lazy freshness), the event plane MAY perform the rebuild itself, because the rebuild is deterministic, model-free, reads only index.jsonl fields, and lands via atomic temp+rename. Index refresh is data plumbing, not aggregation-plane curation; without this carve-out R2's freshness contract and the plane boundary would contradict each other.
...
- Shape: `{ "schema_version": 1, "source": {"index_mtime_ms": N, "index_size": N, "index_sha256": "..."}, "entries": [{"trigger_kind": "phrase"|"tool"|"activity", "value", "episode_id", "summary", "effective_priority", "applies_to_projects", "applies_to_tools", "review_by"}] }`. ALL THREE trigger kinds index explicitly: `trigger_kind` discriminates, `value` carries the phrase, the `tool:` pattern, or the `activity:` class name — no kind is inferred from value syntax, and the contract-mirror pins the discriminator enum. The entry field is named `effective_priority`, never `priority`: the stored frontmatter field (declared 1-7) and the derived band (1-9) are different values with different writers, and one name for both is how consumers confuse them. Expired (`review_by` past) lessons are excluded at build time, and ONLY `status: active` lessons participate: superseded/retired lessons drop from the index, so correcting a wrong lesson via `em-revise` supersession stops the old version firing at the next activation event (the correction path for user-determined-wrong lessons; per-project suppression is for scope errors, not content errors).
...
Install contract (P3, P10, P12): explicit opt-in flag `install.mjs --install-activation`; manifest declares side effects (hook file under `<project>/.claude/hooks/`, one `UserPromptSubmit` registration plus one `PreToolUse` registration (the tool-event contract, R3 matching spec) in `<project>/.claude/settings.json`, ownership ids + checksums); `--uninstall-activation` removes exactly the owned set; round-trip invariant tested like `test-uninstall-enforcement.mjs`. Never global (P12).

Registry reconciliation (explicit amendment, not an implied bump): the RFC-008 R8 registry model currently closes the type set (`plugins/_index.schema.json` enum + `scripts/validate-plugin-registry.mjs` hard-reject unknown types) and names `enforcement` as the only enforcement-layer type. This RFC AMENDS that model: `activation` becomes the SECOND enforcement-layer-hosted type — hook-shaped and per-project like enforcement, but contractually advisory (`blocking: false`, conformance-asserted). Phase 2 deliverables, named: the `plugins/_index.schema.json` enum gains `activation` (MINOR schema-version bump per R8's versioned-contract clause), `validate-plugin-registry.mjs` accepts and validates the new type's manifest sub-schema, CAPABILITIES.md's "Enforcement is not a substrate capability" section gains one sentence noting advisory activation adapters are likewise enforcement-layer-hosted (they hook, therefore they are not substrate — the P12 function test), and RFC-008's R8 section receives a cross-reference note recording the amendment.
...
- `tool:` triggers use the grammar `tool:<ToolName>:<glob>` where `<glob>` supports `*` only; literal `:` or `*` in a phrase is escaped with `\`. `tool:` triggers are matched by the tool-event contract (next bullet), not against prompt text.
- **Tool-event contract (concrete for claude-code; the STRONG tier ships it, not an OQ):** the activation adapter's manifest declares a SECOND registration, `PreToolUse`, in the same `<project>/.claude/settings.json`, same hook file, same conformance clauses (advisory: always exit 0, additionalContext only, `blocking: false`, no decision field on any path — a `tool:` match INFORMS, it never gates, which is what keeps this on the right side of RFC-008 R1). Match input: the event's tool name plus its primary target argument (the command string for Bash, `file_path` for Edit/Write/NotebookEdit) matched against `tool:<ToolName>:<glob>`. Output: the same rendering contract and bounds as prompt matches; the telemetry line records the tool event kind. MEDIUM-tier harnesses map their nearest tool-event surface in their own adapter manifests — OQ-3 tracks only those per-harness mappings; the IO contract and the claude-code registration are fixed here.
...
A separate NON-enforcement SessionStart context hook, shipped by the R3 activation adapter as a sibling settings entry, runs `em-recall` and surfaces its stdout as SessionStart additionalContext, token-bounded (same caps as R3), with a TWO-TIER selection order. **Every activation-layer invocation of `em-recall` (this hook, and any adapter path) is READ-ONLY: it passes `--no-track`** — today's default invocation rewrites index.jsonl access tracking (`scripts/em-recall.mjs:139,475`; runtime-verified: one default recall increments `access_count` and `last_accessed`), which would both breach the event plane's write contract (its writes are telemetry + derived-index refresh, nothing else) and corrupt the R6 conversion metric by counting injection itself as a read. Injection NEVER counts as access; only post-injection agent-initiated tracked reads do. Bounded IO contract for this hook (the event-plane read-boundary made testable, inputs and payload both enumerated): allowed INPUTS are the derived-index family (R2/R10c definition) plus the mechanical, non-episode context inputs em-recall consumes for relevance and the violation preflight — the git branch name, the git remote basename (`git remote get-url origin`, project-name inference), the project name and `keywords` from package.json (directory-basename fallback), and the patterns registry JSON (`patterns/_index.json`, behavior definitions as data, P2) — nothing else, and never an episode `.md`. This enumeration mirrors em-recall's actual context reads and is pinned in the RFC-009 contract-mirror: adding a context input to em-recall without adding it here fails CI, so the list cannot silently drift. Allowed PAYLOAD is the em-recall JSON filtered to an enumerated schema: lesson entries (episode ids, summaries, categories, effective priorities, scores), the context echo, and preflight warnings (themselves derived from index rows plus the patterns registry). Two fixtures pin it: the body-sentinel fixture (a distinctive string planted in an episode BODY never appears in hook output), and a payload-schema fixture (hook output validates against the enumerated schema; unknown keys fail).
...
Replace the manual reviewer-side lesson searches mandated by `scripts/second-opinion/preambles/fragments/review-ladder-v9.4.md:8-14` with dispatcher-side bounded injection: `second-opinion.mjs` queries the trigger index and relevance recall for the review scope (read-only: `--no-track` — dispatcher injection never counts as access, same metric-integrity rule as R4) and prepends matched lesson summaries (same bounds as R3), so review discipline stops depending on the reviewer obeying the preamble. Plane note: the dispatcher is NOT an event-plane hook — it is an on-demand CLI invocation, operator-lifecycle-gated — so the event plane's read boundary does not constrain it; the `--no-track` rule here exists solely to keep the R6 conversion denominator honest. Every dispatch additionally queries `activity:review` (R1/R3), so repo-level review-discipline pointer lessons — ground findings in PRINCIPLES.md / CAPABILITIES.md / the RFC's R-numbers — inject into every codex/subagent dispatch mechanically instead of depending on the operator or dispatching agent remembering them (the recorded 2026-07-05 miss).
...
- **T6 — Migration of existing load-bearing tag conventions**, each with dual-read burn-in: `workplan` becomes `category: workplan` (the workplan-discovery recipe flips after burn-in). `violated:bp-*` moves to a NEW typed scalar frontmatter field on violation episodes — `violated_pattern: <pattern_id>` — written by `em-violation` (which already validates the pattern id at write time; today the id is stored ONLY as a `violated:<pattern_id>` tag, `scripts/em-violation.mjs:125`, runtime-verified — the R1 lesson/violation evidence links carry episode ids, not pattern ids, so they are NOT a replacement); index.jsonl and `em-rebuild-index` carry the field (simple scalar key, zero parser change), `em-recall`'s violation preflight and `em-pattern-health`'s strike counting retarget to it, and the tag remains as a descriptive burn-in shim with a sunset marker. Operator lookup conventions (`agent-prompt`, `canonical-prompt`) are demoted to documented descriptive recipes — nothing mechanical branches on them, so they may remain as search habits.
...
| `em-violation` | CHANGED — R1 `--lesson` linkage flag with write-time validation (existence + `category: lesson`); T6 typed `violated_pattern: <pattern_id>` scalar field (today the pattern id lives only in the `violated:<id>` tag, `scripts/em-violation.mjs:125`) |
...
| `em-pattern-health` | CHANGED — R5 `--hermetic`; T6 strike counting retargets from the `violated:bp-*` tag to the typed `violated_pattern` field |
...
**R2 trigger index:**
- [ ] Lazy build produces per-store `trigger-index.json` with `schema_version`, source fingerprint; second call with unchanged source is a cache hit (no rewrite); a fixture lesson carrying one trigger of EACH kind produces three entries with the correct `trigger_kind` discriminator and `value`
...
- [ ] `--install-activation` writes exactly the manifest-declared set into `<project>/.claude/` (hook file + BOTH registrations: UserPromptSubmit and PreToolUse); `--uninstall-activation` round-trip restores the pre-install state; nothing lands in `~/.claude/` (P12 assert, mock-project E2E)
- [ ] Tool-event fixtures: a Bash event whose command matches `tool:Bash:git push*` fires the lesson with the tool event recorded in telemetry; an Edit event matches on `file_path`; every tool-event code path exits 0 with no decision field (advisory conformance, same gauntlet as prompt matches)
- [ ] Registry amendment: `plugins/_index.json` carrying a `type: activation` entry validates after the schema enum + validator bump (MINOR version); the same entry is REJECTED by the pre-amendment schema (proving the bump is load-bearing, not decorative)
...
- [ ] Read-only invocation: a SessionStart hook run leaves every episode's `access_count` and `last_accessed` byte-unchanged in index.jsonl (the hook passes `--no-track`; injection never counts as access) — fixture mirrors the 2026-07-05 runtime probe that caught the default-tracking write
...
- [ ] T6 migration: workplan discovery works by `--category workplan` post-migration and by the legacy tag during burn-in (dual-read); `em-violation` writes `violated_pattern: <pattern_id>` as a typed scalar that round-trips through `em-rebuild-index`; `em-recall`'s violation preflight and `em-pattern-health`'s strike counting match via `violated_pattern`, not the `violated:bp-*` tag; the tag shim carries a sunset marker

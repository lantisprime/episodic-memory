...
6. **Three-client/one-server end-to-end simulation converges (2026-07-15).** A scratchpad harness implementing this RFC's exchange format, eager reconcile, digest, and pull semantics (episodes created only via real `em-store`/`em-revise`; indexes rebuilt via real `em-rebuild-index`; active sets read via real `em-search`) ran the full lifecycle across `client1-laptop`, `client2-ci`, `client3-web`, and a server replica: (a) backfill — client1 pushed 2 pre-existing episodes, joiners' digests showed `{"behind":2}` and pulls received the full corpus; (b) concurrent fork — clients 1 and 2 revised the same decision ("gRPC" vs "GraphQL") before pushing, and server ingest reported `forks:[{root:…use-rest…, heads:[…graphql…, …grpc…]}]`, which then appeared in client3's session-start digest; (c) resolution — client3, acting as the deciding agent, wrote one resolution episode superseding **both** heads (multi-edge simulated via an `also-supersedes:` line pending PR-0's core support); the next server ingest reported `statusFlips: 2, forks: []`; (d) convergence — after final pulls, all four stores' active sets were byte-identical (`converged: true`: exactly the lesson episode and the resolution episode active everywhere, `digest: {behind: 0, forks: []}`), with the whole disagreement — original, both heads, resolution — preserved as superseded history on every replica. Harness: `sim-sync.mjs` (scratchpad, ~180 lines, stdlib only), transcript in the working session.
7. **No existing check catches forks.** `em-doctor` on the forked store: `{"summary":{"ok":25,"warn":1,"error":0}}` with no chain/multi-head check present (source-confirmed: only `supersedes-links` dangling-pointer warnings exist). Meta-observation from the same working session: the repo's own `negative-scenario-planner` agent **could not run** in the authoring container because its canonical prompt lives as a *global episode* on one maintainer machine — `em-rebuild-index --scope global` reported `count: 0`. The tooling that audits this RFC is itself blocked by the problem this RFC solves.

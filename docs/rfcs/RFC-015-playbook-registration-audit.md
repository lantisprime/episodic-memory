...
> RFC-011 made playbook activation a per-project, operator-authored declaration (`<project>/.episodic-memory/playbooks.json`), but left the whole registration lifecycle invisible: nothing marks an episode as a playbook, nothing tells an author at store time that a playbook is (or is not) declared anywhere, consolidation silently substitutes machine-generated digest text for the curated content a declaration renders (and skips playbook protection on both digest-apply paths), a hand-authored global `playbooks.json` is silently ignored, and an injected READ pointer that goes unread is never surfaced again. This RFC adds an explicit `playbook` frontmatter marker (detection predicate as data, never heuristics; round-tripped through the rebuild whitelist), advisory registration output on `em-store`/`em-revise`/`em-consolidate` (detect and suggest — a write still requires an explicit per-invocation flag, preserving RFC-011 R1's consent model), a consent-carrying `--register-playbook` flag, five read-only `em-doctor` registration-audit checks, closure of the two unprotected consolidation paths, bounded advisory re-surfacing of unread pointers on the existing per-prompt adapter surface, and byte-identical digest body dedup. Everything stays substrate-side and per-project; playbooks and episodes are **advisory-only surfaces by operator lock** — this RFC contains no enforcement and sharply separates playbooks (technical runbooks) from behavior patterns (workflow contracts, a different layer).
...
5. **Registration is necessary but not sufficient — pointer follow-through is invisible.** Registration only gets a pointer rendered; nothing surfaces whether the READ ever happens, and injection stops escalating. Live evidence, 2026-07-26 (the same session that produced this RFC): the a6c2 lesson's `access_count` sat frozen at 73 across nine consecutive injections (activation-log `access_count_at_inject: 73` at 13:34Z, 13:40Z, 13:46Z, 21:38Z, 21:49Z, 21:51Z, 23:29Z, 23:32Z, 23:34Z on 2026-07-25) — nine deliveries, zero reads; and v15, registered mid-session, was read by the orchestrator only after two operator prompts, because a `session_start`-mode playbook registered mid-session surfaces nowhere until the next session start (RFC-011 R3 renders playbooks at session start only; probe-confirmed round 2). RFC-011 R7 already tracks the read side (`--read` bumps `access_count`); the inject side is logged with a per-inject `access_count_at_inject` snapshot (RFC-009 R6 telemetry). The conversion join exists in the data, end-to-end and probe-verified, and is surfaced to no one.
6. **Consolidated digests degrade the read they point at.** The v15 digest body carries its content five times over — 8 content blocks, each appearing 5 times, 26,911 bytes (probe-measured) — because digest assembly concatenates member bodies with no content dedup. The members are revision-chain variants: of the five bodies, exactly two are byte-identical (round-2 hash evidence), the rest near-identical. The registered pointer works, but every follow-through read pays roughly five times the tokens, directly taxing the exact behavior Problems 1-5 are trying to increase.
...
- **(a) Marker inheritance.** A digest whose members include a marker-bearing episode is itself written with `playbook: true`. On the clerk `dedupe` action no digest is written — members are superseded onto an existing canonical episode — so there is no new episode to mark and no existing episode is mutated (B-4). The advisory names the canonical id as the resolution target instead. Because canonical selection would otherwise be cluster-build order, a marker-bearing member is preferred as canonical when the cluster contains one: otherwise a marked member is superseded onto an unmarked canonical and the R1 detection predicate silently desyncs, since the chain terminal would no longer carry `playbook: true`.
- **(b) Protection parity.** The legacy digest-apply transaction (`em-consolidate.mjs:1057-1179`) gains the same `resolvePlaybookProtection` consultation the fold paths already have; the clerk-apply call site (`:1863`) passes real resolved playbook ids instead of the hardcoded `[]`, so class-e protection (`protection.mjs:245-267`) fires on every archival path. This **extends** RFC-011 R5(b)'s binding rather than mirroring it, and the extension is wider than a call-site count: R5(b) bound **archival** consumers (`em-prune`, `em-consolidate --fold-superseded`), and the two apply paths do not archive at all — they supersede in place (`em-consolidate.mjs:1105-1112`, `:1253-1280`), while only `foldStore` (`:254-300`) moves files into `archived/`. R4b therefore extends R5(b) from archival consumers to **supersession** consumers, and adds a consumer-registry read on `--scope global` applies that those paths did not previously perform. It also widens the accepted denial-of-curation residual: once protection fires on the apply paths, a declared chain's own digest is re-anchored by class-e and its members by class-c, so the chain cannot be re-consolidated until the operator removes the declaration from `playbooks.json`. There is no override flag; `--include-pinned` gates only the pinned-row filter.
...
Five read-only checks in the existing `report(id, scope, level, message, extra)` shape (`scripts/em-doctor.mjs:96-98`) — the four below plus R7c's `playbook-unread`; per-store checks ride `checkStore` (`:124`) via `checkStoreWithDir` (`:586`; `--all-projects` registry loop `:601-605`), one-shot checks the standalone block (`:608-612`):
...
- **(a) Re-surfacing.** The activation adapter re-renders an unread `session_start` playbook pointer on subsequent `UserPromptSubmit` events — bounded to at most one playbook line per prompt event, round-robin by least-recently-surfaced when several are unread (no starvation), counted inside the existing 500-token render budget (`activation-hook-run.mjs:592`), and suppressed once the read lands. *Unread* is the **count-based join** (round-2 N2): the playbook's current `access_count` on its trigger-index row is less than or equal to the `access_count_at_inject` snapshot of its latest `session_start`/`per_prompt` inject row in the activation log (equality is the normal case; `<=` rather than `==` so a backward-moved counter — index rollback or corruption — fails OPEN to an extra advisory render rather than suppressing the pointer permanently) — playbook index rows carry no `last_accessed`, and the event plane may not read `index.jsonl`, so the count form is the only join computable from sanctioned reads (probe-verified end-to-end). Re-surfaced renders append their own inject rows (so R7c counts them). Suppression timing is rebuild-dependent: the read lands on the index row, which the event plane sees after the freshness-triggered rebuild — one extra render in that window is acceptable advisory behavior. No inject rows for a registered playbook (fresh registration, or a rotated log) means *unread by definition*: it surfaces on the very next prompt, which closes the mid-session registration gap by construction.
- **(b) Read-boundary amendment, stated explicitly — with its full package.** The RFC-009 R4 strict read boundary (stated as REQ-19 in the P2 plan, `docs/plans/rfc-009-p2.md:67`; the identifier does not appear in RFC-009 itself) confines event-time reads to the trigger index plus `lesson-suppress.json`; R7a adds the activation log tail (`.episodic-memory/activation-log.jsonl`, written by this same adapter under RFC-009 R6) as a third sanctioned event-time read. Same store, stat-then-tail bounded; **not** append-only in general — the clerk rotates and consumes it (`em-consolidate.mjs:1806-2045`, `clerkComputeConversion` at `:1964`, atomic rename at `:2023`), so all R7 joins are per-rotation-window (see R7c). Fail-open: an absent or unparseable log disables re-surfacing, never injection. RFC-009 `:47` forbids loosening this boundary as a quiet patch, so P1-S3 ships the whole amendment package: the RFC-009 text revision, the contract-mirror update (`tests/test-contract-mirror-009.mjs`), and the environment-independence/payload-schema fixture updates, in lockstep.
- **(c) Doctor check `playbook-unread`** (joins the R5 table): warns when a declared playbook's inject count since its last read meets a threshold (fixed at 3 for P1; configurability deferred) — the a6c2 signature (nine injects, zero reads) becomes an operator-visible finding instead of a frozen counter nobody joins. Counts are **within-rotation-window lower bounds** (a streak straddling a clerk rotation undercounts; stated, accepted). Playbook `session_start` log rows currently carry `source_scope: null` (the stamp misses the playbooks trio), so R7c joins per episode id only, never cross-store, until that is stamped.
...
`em-consolidate` digest assembly dedups member bodies by content before concatenation: byte-identical member bodies collapse to one copy, with the member provenance list (`(id: ..., date)` headers) preserved for every member. Applies to both digest-write paths (legacy body concatenation `em-consolidate.mjs:1075-1079`, clerk body assembly `:1493-1498`). **Honest outcome on the motivating instance (round-2 N1):** the v15 members are revision-chain variants — two of five bodies byte-identical — so byte-identical dedup collapses that digest 5→4 (~20% smaller), not to one edition. Near-identical bodies (the bulk of v15's 5x duplication) are out of scope for P1 — only byte-identical dedup ships, the conservative half that cannot lose content; similarity-based dedup for revision-chain digests is OQ-4. T12 pins the 5→4 expectation on a v15-shaped fixture.
...
**In scope:** the marker (with rebuild round-trip); advisories on store/revise/consolidate; `--register-playbook`; five doctor checks; R4 consolidation closure; R7 advisory re-surfacing; R8 byte-identical digest dedup.

**Non-goals:** auto-registration without a flag (B-2); a global `playbooks.json` contract; marker-based prune protection (protection remains declaration-driven per RFC-011 R5 — an undeclared marker-bearing episode is exactly what `playbook-unregistered` surfaces); any activation-adapter **enforcement or new adapter/hook registration** (the R7a-b advisory render legs on the already-registered adapter are the only adapter change); similarity-based digest dedup (OQ-4); any gating (B-1). `em-consolidate --clerk --enrich --apply` is out of scope for R4: it revises through `em-revise` and archives nothing, and RFC-011 R5(b) binds archival consumers.
...
One phase, five slices:

- **P1-S1** — R1 marker (incl. the `em-rebuild-index` whitelist lockstep) + R2 advisories + R3 flag + R5 doctor checks, with their acceptance fixtures; docs (CLAUDE.md testing block, script `--help`) in the same PR.
- **P1-S2** — R4 consolidation closure (protection parity on both apply paths, marker inheritance, substitution advisory, R4d fail-closed aborts). This slice **folds GitHub issue #610** (the standalone protection-gap defect report); #610 is closed against this RFC and tracked here, not as an issue.
...
- **P1-S4** — R8 byte-identical digest body dedup in both `em-consolidate` digest-write paths (folds the 2026-07-26 v15 duplicate-digest finding; honest 5→4 outcome per round-2 N1).
- **P1-S5** — pre-existing substrate hardening discovered by round-2 fixtures (N8): `isMainModule()` (`em-trigger-index.mjs:996`) compares `import.meta.url` to `argv[1]` without realpath, so symlinked invocation paths silently run no main (exit 0, empty output) and the hook's carve-out rebuild degrades to stale cache. Realpath both sides; add a symlink-invocation test leg.
...
| T7 | doctor: marker-bearing undeclared terminal / declared-entry failing check-time resolution / declared id resolving to a digest other than itself / global `playbooks.json` present | each of the four checks fires warn with the right id (substitution check matches its R5 definition exactly — any resolve-to-digest, declared or not; recomputed never from cached build_report); all four silent on a clean fixture (both polarities) |
...
| T10 | unread re-surfacing | after a session_start inject with no subsequent read, the next prompt event re-renders the pointer (at most one playbook line per event, count-based join); the re-surfaced render appends its own inject row (observed in the log); two unread playbooks alternate round-robin across events; a tracked `--read` suppresses re-surfacing after the next rebuild; registering a playbook mid-session surfaces it on the next prompt without a session restart |
...
| T12 | digest dedup | consolidating five byte-identical member bodies yields one body copy + five provenance headers (both digest paths); non-identical bodies remain fully concatenated; a v15-shaped fixture (two byte-identical + three near-identical members) collapses exactly 5→4 (pins the honest R8 outcome, round-2 N1); digest read round-trips byte-exact for the surviving content |
...
| Slice | Status | Commits | PR | Notes |
|---|---|---|---|---|
| P1-S1 | implemented, 3-round GLM-5.2 ACCEPT at `97976a3` | `627bcc6` (slice), `1afbf8f` (r1 fold + F9), `97976a3` (r2 hygiene fold) | #613 | Build spec `docs/plans/rfc-015-p1-s1.md` (NSP HOLD folded pre-build: lock-span registration, 64 KiB writer bound, symlink disposition). Suite `tests/test-rfc-015-registration.mjs` 20/20; residual noted in review r3: registered-store scans evaluate cwd-local declarations in the health/substitution checks — pre-existing pattern, build-parity, candidate for a later amendment. |
...
- **RFC-009 (accepted)** — activation surfaces that render declared playbooks. **Revised by this RFC:** R7b amends the R4 strict read boundary (activation-log tail as a third sanctioned event-time read), shipped with the mandated text-revision + contract-mirror + fixture package (P1-S3), never as a quiet patch (RFC-009 `:47`).
...
|---|---|---|---|---|
| 1 | pi kimi-k3 (herdr seat, operator-directed; frozen `e8ff52d`) | HOLD-13 | F1 P1 premise falsification (declared ids resolve forward through `superseded_by` to the digest — probe-confirmed on fixtures + real store; real defect is content substitution), F2 P1 `em-rebuild-index` whitelist drops the marker, F3-F6 P2 (two-caps split; corrupt-file refusal; clerk-path abort semantics; unregistered-check scope), F7-F10 P3, F11-F13 NIT. All anchors verified against disk (2 loose ranges). | 13/13 ACCEPT, folded 2026-07-26: Problem 2 re-diagnosed on the substitution framing; R4c→substitution advisory, R4d fail-closed; R5 dangling check→`playbook-content-substitution`, health check recomputes; R3 full consent contract; R1 rebuild lockstep; F7 exported helper; T3/T5/T6/T7 rewritten, T14-T16 added; OQ-1/OQ-3 updated. |
| 2 | pi kimi-k3 (same seat; frozen `1b6b97c`) | HOLD-8 | Task A: 12/13 folds faithful, F1 fold incomplete in scaffolding. Task B (first look at R7/R8/taxonomy/lock): N1 P1 R8 promise vs scope (v15 members revision-chain variants, only 2/5 byte-identical → 5→4 not 5→1; hash evidence), N2 P2 unread join must be count-based (playbook rows carry no `last_accessed`; count join probe-verified), N3 P2 REQ-19 amendment package under-scoped (cite resolves only in the P2 plan; contract-mirror + fixtures make quiet patches CI-red; Related-RFCs contradiction), N4 P2 scaffolding contradictions (AI context, Non-goals, slice count, S2 wording, check count), N5-N6 P3 (log rotation semantics; re-surface logging/ordering/budget/threshold), N7-N8 NIT (anchors; nine not seven injects; pre-existing `isMainModule` symlink defect). Consent walk on new sections: clean; taxonomy + operator lock internally consistent. | 8/8 ACCEPT (N1 as ACCEPT-WITH-MOD: honest-outcome correction, scope unchanged, OQ-4 opened), folded 2026-07-26: AI context rewritten; R7a count-based join + round-robin + budget + logging semantics; R7b full RFC-009 package + rotation honesty; R7c window bounds + threshold pinned + scope note; R8 honest 5→4 + corrected anchors; slices five incl. P1-S5 (`isMainModule` realpath hardening); T7/T10/T12 legs; Related RFCs corrected; Problem 5 nine injects. |
| 3 | pi kimi-k3 (same seat; frozen `0e05539`, fold-verification only) | HOLD-1 → **ACCEPT** (closure on `0e65692`) | All eight r2 folds verified faithful and complete (anchors re-verified on disk); F1 scaffolding remnants confirmed gone; one fresh NIT (R2-NIT-1: coverage-table "behavior identical" clause contradicted P1-S5's intentional `isMainModule` change in the same file). | R2-NIT-1 fixed in `0e65692` (disposition now discloses both the F7 export and the S5 behavior change); closure round confirmed the clause and returned ACCEPT. Seat note: an unnecessary `git checkout` of the frozen ref in r3 detached the worktree HEAD (no content impact; branch fast-forwarded and reattached). |
| P1-S1 r1 | pi glm-5.2 (neuralwatt, herdr seat; frozen `627bcc6`) | HOLD-8 | F1 P1 doctor `--all-projects` TDZ crash (probe: ReferenceError, 3/4 scope variants), F2 P1 doctor false-positive on global-marker-declared-locally + doctor-vs-build parity break, F3-F5 P2 (empty-trigger write-time advisory missing/T16 weakened; em-revise scope refusal post-write; advisory cannot resolve global-store markers), F6-F8 NIT. All probe-backed on isolated fixtures. | 8/8 ACCEPT (F6 WITH-MOD), folded in `1afbf8f` per `docs/plans/rfc-015-p1-s1-fold-r1.md`; orchestrator added F9 P1 (real-machine repro: F2 fix over-merged, marker scan counted cwd markers in every store's scan) — scan scoped to the scanned store, t7e. |
| P1-S1 r2 | pi glm-5.2 (same seat; frozen `1afbf8f`) | HOLD (0 P1, 0 P2 — hygiene) | All r1 P1/P2 folds verified fixed by probe; NF1 P3 merge helper reimplemented instead of exported (F7 drift class), NF3 P3 `--all-projects` union loop never exercised non-empty, NF2/NF4 NIT, F8-r2 NIT (brace re-indent missed). | 5/5 folded in `97976a3` (NF4 in minimal single-merge form); t7c2 union leg added. |
| P1-S1 r3 | pi glm-5.2 (same seat; frozen `97976a3`, closure) | **ACCEPT** | All r2 folds faithful; F2/F9 regression-free (probes); builder's registered-store resolution-row call (scanned+global) ruled R5-correct and build-parity; B-1 posture clean on every leg. One pre-existing out-of-scope note: registered-store scans evaluate cwd-local declarations in health/substitution checks (predates slice, build-parity). | Closure; residual recorded in the Implementation row and PR #613 body. |
...
| OQ-3 | Should `playbook-unregistered` under `--all-projects` consult every registered store's declarations before warning? | champion | closed — R5 defines the declaration set: cwd project under default scope (limitation named in the message), registry-wide union under `--all-projects` |
| OQ-4 | Similarity-based digest dedup for revision-chain digests (the bulk of v15's duplication, which byte-identical R8 leaves in place) — worth a follow-on slice, and under what similarity contract? | champion | open |

...
1. **Decision logic in core, hook is a thin wrapper.** The block-or-allow decision lives in `scripts/em-recall.mjs` as `--gate stop`. The shell hook reads stdin, honors the `stop_hook_active` infinite-loop guard (per cached docs `claude-code-hooks-guide.md:421-431`), invokes the core script, and passes JSON through (with a fail-loud envelope on script absence/error). [Phase 2:404](#phase-2-claude-code-reference-adapter) will relocate the shell wrapper into `adapters/claude-code/capabilities/enforcement.mjs`; the core `--gate` flag remains in core unchanged (P9 — core never imports adapters).
   ...
   > ⚠️ **Partially superseded by [RFC-008](RFC-008-decouple-enforcement-from-substrate.md) (2026-05-27).** This commitment — gate decision logic stays inside `em-recall.mjs` core — is reversed by RFC-008 (R1). RFC-008 extracts `--gate`, markers, carve-outs, and the classifier OUT to a separate enforcement thin waist (`enforce-contract.mjs` + `patterns/*.json`), returning `em-recall.mjs` to pure recall; keeping enforcement in the recall core is a soft P9/P1 violation that this RFC deferred. The rest of RFC-003 (adapter contract, `installed_state` manifest, cross-tool messaging) stands and is reused by RFC-008 for migration detection (R8). RFC-003's frontmatter status is unchanged.

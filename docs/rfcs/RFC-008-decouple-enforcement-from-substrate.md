...
**Cross-ref (RFC-009 R3, the first additive-MINOR extension):** [RFC-009 (Lesson Activation)](RFC-009-lesson-activation.md) registers a fifth `type`, **`activation`** — the per-project, opt-in, advisory event-plane adapter that surfaces lesson pointers from the derived trigger index. It ships the full per-type contract this clause requires (registry sub-schema, `activationDescriptor` + activation manifest schema, runtime-IO schema, and the activation conformance gauntlet) with `blocking: {const: false}`, and lands as exactly the additive-MINOR schema-version bump described here. It sits on the substrate/advisory side (never enforcement): every hook exits 0 with no decision channel.

**v11.9 versioned-contract clause (R8 contract-evolution; backward/forward compatibility):** The registry (`plugins/_index.json`) and its meta-schema MUST carry a REQUIRED **`schema_version`** (semver), mapped to BOTH the schema (which declares the contract version it defines) and the registry validator (`validate-plugin-registry.mjs`, which enforces it). An unversioned registry is **hard-rejected** (no version ⇒ no validation, never a silent default). The validator supports a version **range**, not an exact pin: **backward** (registry version ≤ the validator's max within the same major ⇒ validate with that version's rules; a newer validator still reads older registries), **forward** (registry version > the validator's max — a higher MAJOR **or** a newer MINOR within the same major ⇒ **fail closed** with "registry written against newer contract vX; upgrade the validator" — never silently mis-validate an unknown contract; an under-upgraded validator cannot honour a newer minor's additions, and top-level `additionalProperties:false` is the *static* enforcement of this for any unknown top-level key), and **additive minor** (a new plugin kind / new optional field within the same major ⇒ accepted by construction **by a validator that supports that minor** — i.e. additive-MINOR is a forward-superset: the newer validator+schema is a superset that reads older same-major registries; it never means an under-upgraded validator accepts newer data, which is the forward fail-closed case above). A **MINOR** bump is additive-only and backward-compatible; a **MAJOR** bump is breaking (removed/renamed field, tightened `required`, changed enum semantics). This is what makes "register a future plugin kind" a safe, validated MINOR bump rather than an ad-hoc edit to a merged contract. A single `CURRENT_SCHEMA_VERSION` constant is asserted byte-equal across the schema, the validator's max-supported version, and the golden fixtures, with a CI check that fails on divergence.
...
The remaining four payload shapes are **locked here in P0** (not deferred to plugin authors — deferral is the exact F37 failure this section prevents). Each field reflects what the enforcement layer needs to make a decision at that event. **Cross-event invariants** (apply to every payload below + `event-pre-tool-use`): `additionalProperties: false`; `cwd` is `{ "type": "string", "minLength": 1 }` (canonicalized absolute, `pwd -P` equivalent, per F25); `session_id` is `{ "type": "string", "minLength": 1 }`; `timestamp_iso8601` is `{ "type": "string", "format": "date-time" }`; `turn_index` is `{ "type": "integer", "minimum": 0 }`; `tool_args` is `{ "type": ["object", "null"] }`; `harness` reuses the closed M3 enum `{claude-code, opencode, codex, pi-agent, cursor, windsurf}`.

| Schema (`schemas/events/…`) | `required` | optional | Rationale (anchor) |
|---|---|---|---|
| `event-pre-tool-use.schema.json` | `tool`, `tool_args`, `cwd`, `session_id`, `turn_index`, `timestamp_iso8601` | `tool_args_raw` | Representative shape above (R3, R4) |
| `event-tool-result.schema.json` | `tool`, `tool_args`, `result`, `cwd`, `session_id`, `turn_index`, `timestamp_iso8601` | `exit_code` (`["integer","null"]`), `is_error` (boolean) | `tool_result` STRONG = modify/observe the result before the model sees it (events.json action) — needs the `result` plus what produced it |
| `event-stop.schema.json` | `cwd`, `session_id`, `turn_index`, `timestamp_iso8601`, `is_subagent` (boolean) | `stop_source` (string), `stop_hook_active` (boolean) | Stop-gate reads marker state but must resolve session+repo+turn; `is_subagent` distinguishes Stop vs SubagentStop (both gated); `stop_hook_active` prevents re-entry loops |
| `event-session-start.schema.json` | `cwd`, `session_id`, `timestamp_iso8601`, `harness` | `project_root`, `source` (enum `{startup, resume, clear}`) | `inject_context` binds at session start; `project_root` optional because it may require resolution from `cwd` |
| `event-session-end.schema.json` | `cwd`, `session_id`, `timestamp_iso8601`, `end_reason` (enum `{clear, logout, error, other}`) | `project_root`, `turn_index` | `write_artifact` at end; MEDIUM "artifact may be missing on abnormal termination" ⇒ `end_reason` is required so the artifact writer can branch on clean vs abnormal |

`result` is typed `{ "type": ["string", "object", "null"] }` (harness-specific). These five shapes are the canonical post-translation payloads every adapter's `event_translations.field_bindings` (M5c) MUST produce; `validate-plugin-registry.mjs` (P1) replays recorded fixtures against them (F39 Layer 1).
...
**Detection-layer attribution (v11.8).** P0 stages every fixture but does NOT detect most of them — only failures expressible in pure JSON-Schema 2020-12 are P0-detectable; the rest are caught by later validators. A sibling manifest `tests/fixtures/plugins/_corpus-index.json` annotates each fixture with `_detected_by ∈ {p0-schema, p1-registry-validator, p2-cross-file-validator, p3-runtime}` so the corpus never implies P0 catches a failure only a later layer can. Examples: `bad-extra-field` / `bad-unknown-harness` / `bad-invalid-tier` / `bad-default-mode-with-override-path` / `bad-alert-both-vocab-fields-set` / `bad-alert-no-vocab-field` / `bad-runbook-command-shape-backslash-path` = **p0-schema**; `bad-stale-taxonomy-version` / `bad-dangling-label` / `bad-renamed-harness` / `bad-missing-runbook` / `bad-capability-dishonest` / `bad-missing-bypass-record` / path-traversal + symlink-escape / regex-malformed + inline-secret-ref = **p1-registry-validator**; `bad-override-emits-tsv` / `bad-runbook-adapter-private-log-leak` = **p3-runtime**. `_corpus-index.json` also records the canonical hash-serialization contract used to bake `good-manifest.json`'s `taxonomy_version`/`events_version` (F8/F37), so P2's validator reuses the identical serialization.
...
| F66 | P1 | **[R6, R10]** `log_paths` authority root unspecified — entries didn't say whether they resolve against `project_root`/`store_root`/plugin-dir/HOME, so gauntlet step 9 could scan one root while the adapter writes another (codex round-4, cap) | §9 `log_paths` closure: entries are absolute OR relative-to-`project_root` (M7e-normalized to absolute); installed-runtime/HOME entries written absolute; adapter MUST write to exactly the normalized path(s); step 9 scans the same normalized locations — scan root ≡ write root by construction; never resolved against caller cwd or `store_root`. |
| F67 | P2 | **[R1, R3, R4]** one stale normative sentence (step-6 alert) still said episodes "MUST land under `<projectRoot>/.episodic-memory/`", contradicting the F61 `store_root` closure — cross-reference drift (codex round-4, cap) | line updated to `store_root/.episodic-memory/` with the `store_root = resolveRepoRoot(projectRoot)` expansion (normal checkout ≡ projectRoot; worktree → main; submodule → own toplevel). |
...
| **P2** | BP contract instances + contract validators | 39 | 5 | ~35K | P0 | R2, R3, R4 |
...
| F5 | P1 | no meta-validation of validators | `taxonomy.schema.json` + `validate-taxonomy-schema.mjs` + golden tests *(P2: folded into `validate-bp-contract.mjs` assertions 1 + 9 plus the `validate-schemas.mjs` doc-validity linter; the standalone `validate-taxonomy-schema.mjs` was dropped — F-6)* |
...
- **Round 4 — codex**, request `20260528-232722-rfc-008-v11-5-round-4-final-post-hold-fi-aac0`, reply `20260528-233020-reply-codex-to-20260528-232722-rfc-008-v-42dc`: **HOLD at the 4-round cap.** F64 + F65 confirmed closed. 2 findings, both trivial last-mile consistency drift: F66 (P1, R6/R10 — `log_paths` authority root unspecified), F67 (P2, R1/R3/R4 — one stale `<projectRoot>` alert-location line missed in the F61 sweep). Codex explicitly: *"HOLD at round 4 cap. Do not spawn round 5; consult the champion."* **Both ACCEPT and folded inline as v11.6.** Per the round-cap discipline, NO round 5 is auto-dispatched — escalated to the champion.

**Cap status (champion decision pending):** the review converged monotonically — round-1 8 findings → round-2 3 → round-3 2 → round-4 2, and from round 2 onward every finding was either pre-flagged by the implementer's own second-order review or was internal-consistency drift introduced by the immediately-prior fix (never an architectural objection; R1–R10 never challenged). F66/F67 are folded in v11.6. The trajectory is convergence on shrinking trivial drift, not unresolved design risk. **Champion options:** (a) accept the spec as folded (v11.6) and merge PR #365 — the standing recommendation, given the convergence shape; (b) authorize one confirmation round (round 5) on the v11.6 delta if a clean ACCEPT verdict on the record is wanted before merge.

...
| `emitted_label` | string | YES iff `alert_type == "classifier_out_of_vocabulary"` | the rejected label literal (`null` for an event-vocabulary alert) |
| `emitted_event_id` | string\|null | YES iff `alert_type == "event_out_of_vocabulary"` | the rejected event id (`null` for a label-vocabulary alert) — **F55/F44 closure (v11.3)** |
| `events_version` | string\|null | YES iff `alert_type == "event_out_of_vocabulary"` | loaded `events.json` sha256 at rejection time (`null` for a label alert) |
...
- **Round 2 — pending dispatch (v11.3).** Same provider/cap/mode. Scope: the F52–F60 fixes above. Reply will be appended; expect ACCEPT.

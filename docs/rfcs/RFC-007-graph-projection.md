...
| Phase 3 — PR #617 (`dafc3c8`) | `scripts/em-graph.mjs` (extend), `tests/test-em-graph-links.mjs` (new, 31 cases), `.github/workflows/tests.yml` (new CI step), `docs/rfcs/RFC-007-graph-projection.md` (status rows, contradictions, Target resolution), `docs/plans/RFC-007-P3-wiki-link-composes-with.md` (new) | suite CI-wired at `.github/workflows/tests.yml:407-408`; `tests/test-em-graph.mjs` 10/10 and `tests/test-em-graph-nodes.mjs` 28/28 unchanged; default output verified byte-identical to `8e6e699` by `cmp` on `--hubs` and `--orphans` | `wiki-link` + `composes-with` edges (opt-in, absent from `DEFAULT_EDGES`); `slugify()` becomes load-bearing for edge targets; per-edge-type `dangling[]`; filename-basename **alias index** for target resolution only (node ids stay frontmatter-derived — adopted after measuring 13/132 and 0/76 resolution under the literal spec); rule nodes integrated into `--orphans` (degree-aware), `--hubs` (degree seeding) and `--from` (BFS fall-through); the two RFC self-contradictions settled (bucket name, regex form) |

...
- **Two SessionStart hooks** (UPDATED v3.16) — `bp1-approval-check.sh` (H1, fires FIRST per §559 H-cfg ordering; shipped slice 2d-R) and `bp1-sweep-on-session.sh` (H2, fires AFTER H1; activation-gated; provides fallback liveness when `mcp__scheduled-tasks` capability missing). Both hooks always exit 0 per §178 four-mode silent-exit contract.
...
| `.claude/hooks/bp1-approval-check.sh` hook (H1, shipped slice 2d-R) | M2 | Activation-gated; flag-check refusal exits 0 silently. See §178 four-mode silent-exit contract below. |
...
|---|---|---|---|
| H1 (shipped v3.16, slice 2d-R) | `.claude/hooks/bp1-approval-check.sh` | SessionStart, FIRST in array per §559 H-cfg ordering | Scans `<canonical_root>/.checkpoints/bp1-approval-*.json`; calls `bp1-marker-validate.mjs` per marker; on `ok + expired=true` → `bp1-orchestrator.mjs confirm-approval --outcome auto_approved`; on `invalid` → `bp1-emit-marker-invalid-evidence.mjs` (Case A signed / Case B stderr); on unparseable filename → Case C hook-stderr. **Activation-gated** (calls `bp1-flag-check` BEFORE marker scan; resolves canonical project root via `git rev-parse --show-toplevel` BEFORE flag-check so the activation map's canonical-root key matches even when SessionStart fires from a subdir / linked worktree). Always exits 0 per §178 four-mode silent-exit contract. |
| H2 (NEW v3.10) | `.claude/hooks/bp1-sweep-on-session.sh` | SessionStart (after H1) | Calls `bp1-deadline-sweep.mjs --once` when `scheduled_tasks_capability == fallback`. **Activation-gated** (calls `bp1-flag-check` first; no-op if flag-check fails). Best-effort liveness for T1 + T1b when scheduled-tasks unavailable. Added v3.10 per CLI v3.9 F2. Always exits 0 per §178 four-mode silent-exit contract. |
...
### §178 SessionStart hook silent-exit contract (NEW v3.16, slice 2d-R)

Both SessionStart hooks (H1 `bp1-approval-check.sh` and H2 `bp1-sweep-on-session.sh`) MUST always exit 0. A non-zero exit from a SessionStart hook would fail the user's Claude session start — bp1 is a best-effort liveness layer, never a session-blocker. The contract enumerates every observable mode and the matching disposition:

| # | Mode | Disposition | Exit |
|---|---|---|---|
| 1 | bp1 inert for project (flag-check refuses) | Silent no-op (no stderr; flag-check called with `--no-emit` so no per-session audit episode) | 0 |
| 2 | `.checkpoints/` directory absent | Silent no-op | 0 |
| 3 | Marker file race (filename listed by `find`, then concurrent unlink before validate) → validate `status: missing` | Silent no-op | 0 |
...
**Filename regex authority (§566 closure):** the writer (`bp1-marker.mjs:markerPath`) and reader (H1 hook) MUST agree on the filename shape. The reader matches `bp1-approval-(<run_id>).json` with `run_id =~ /^bp1-run-[a-z0-9-]+$/`. Filenames that match the glob `bp1-approval-*.json` but fail the run_id sub-regex go to Case C, NOT Case A/B (validator never gets called).

**§559 H-cfg ordering invariant:** H1 is wired BEFORE H2 in `.claude/settings.json`'s SessionStart hooks array. `mergeSessionStartH1Hook` (in `bp1-install-helpers.mjs`) finds the H2 entry by basename match and `arr.splice(h2Index, 0, h1Entry)` inserts H1 immediately before H2 (relative ordering, not absolute index). On installs without H2, H1 appends to the end of the array. Pre-existing unrelated SessionStart entries are preserved.

**§646 linked-worktree symmetry:** Writer (`record-awaiting-approval`) and reader (H1 hook) both canonicalize the project root via `git rev-parse --show-toplevel` resolved at the originating CWD. In a linked worktree, this returns the WORKTREE root (not the main checkout's root), and the marker lands under `<worktree_root>/.checkpoints/`. Reader scans the same dir. Linked-worktree fixtures are covered by `tests/test-bp1-approval-check-hook.mjs` AC-12 (subdir-CWD test exercises the identical resolution code path).
...
| PR-1c-A + PR-1c-B (M1 — Foundation) — PR #200 (`2827ec3`) + PR #206 (`40c8e02`) | #200: `scripts/bp1-orchestrator.mjs` (new, `init-run`), `scripts/bp1-canonicalize.mjs` (new), `scripts/lib/bp1-canonicalize.mjs`, `scripts/lib/bp1-hmac.mjs`, `scripts/lib/bp1-keys.mjs`, `scripts/lib/bp1-run-state.mjs` (all new), `scripts/validate-rfc-canonical-fields.mjs` (new), `install.mjs`, `.github/workflows/rfc-validate.yml`, this RFC; #206: `scripts/bp1-orchestrator.mjs` (finalize-replay), `scripts/lib/bp1-frontmatter.mjs` (new), `scripts/lib/bp1-manifest.mjs` | #200: `tests/test-bp1-canonicalize.mjs`, `tests/test-bp1-hmac-live.mjs`, `tests/test-bp1-orchestrator-init-run.mjs`, `tests/test-bp1-run-state.mjs`, `tests/test-install-bp1-runkey-gitignore.mjs` (all new); #206: `tests/test-bp1-finalize-run.mjs`, `tests/test-bp1-frontmatter.mjs`, `tests/test-bp1-hmac-manifest.mjs`, `tests/test-bp1-manifest-collect.mjs` (all new) | _shipped_ ✓ (row backfilled from `git show --stat` on both merge commits). M1 BP-1 Foundation: `init-run` + HMAC + canonicalize + run-state (#200) and finalize-replay slice 2 (#206). The v3.13 changelog entry below records the same two PRs; RFC text caught up to the shipped code per the resolutions in decision episode `20260507-113139-m1-rfc-ambiguity-resolutions-before-plan-2e12`. |

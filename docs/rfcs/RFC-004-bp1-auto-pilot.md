...
| **H17** | **Finalize crashes between step 5 (manifest disk re-read fence) and step 7 (mark terminal)** | recovery: manifest exists; replay still works; `bp1-finalize-recover` branches on key presence — State A (key still on disk, post-step-5 / pre-step-6 crash) re-runs shred then markTerminal; State B (key already shredded, post-step-6 / pre-step-7 crash) idempotent markTerminal; State D (key damaged) unlinks then markTerminal. The shipped step ordering (shred at 6, markTerminal at 7) preserves I-4 across both crash points — see §815 invariant-preservation note. | crash |

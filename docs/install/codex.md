# Install episodic-memory for Codex (OpenAI)

Self-contained guide. Everything you need is here.

## WHEN TO USE
- You are setting up episodic memory for a Codex project.
...
Installed Codex skill to <PROJ>/.agents/skills/episodic-memory/SKILL.md
...
node ~/.episodic-memory/scripts/em-search.mjs --project <name> --no-track --no-score
```

`em-store` prints `{"status":"ok","id":"...","file":"...","scope":"local"}`.
`em-search` prints `{"status":"ok","count":N,"episodes":[...]}`.

## Harness-specific traps

- Codex discovers skills by `SKILL.md` inside the skill directory. Arbitrary other
  markdown filenames in that directory are inert.
- Codex and Pi Agent share this exact destination:

...
| #17532 firing (real codex/tmux) | `manual: node tests/integration/codex-tmux-e2e.mjs` (full suite) | **S4 observed (real codex 0.142.3):** full suite `3/3 pass` — `ok - firingProof` (PreToolUse fired + written), `ok - trustGate` (untrusted → no fire), `ok - enforcementProof` (deny/allow pair). |
| Enforcement (real codex/tmux) | `manual: node tests/integration/codex-tmux-e2e.mjs enforcementProof` | **S4 observed (real codex 0.142.3):** `ok - enforcementProof` (1/1 pass) — deployed adapter DENIED `src/probe.mjs` (repo-source) AND ALLOWED `docs/plans/note.md` (discriminating pair; `src/probe.mjs` absent on disk). Harness `navigateStartup` loop added to tolerate codex startup-prompt drift (update notice + variable dir-/hook-trust order). |

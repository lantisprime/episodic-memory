...
| 3.10 | — | — | Inspect `git diff --check`, `git diff --stat`, and the full diff against the allowed file set. | diff contains only the 12 listed files; no `scripts/em-promote.mjs` byte differs |
...
|---|---|---|
| CHANGED | `validate-plugin-registry.mjs`, `_index.schema.json`, `_index.json`, 4 version fixtures, `test-em-promote.mjs` | slot + L-checks + MINOR bump + gauntlet |
| INTERACTS, oracle-coupled (read-only tests) | `test-plugin-registry.mjs`, `test-p0-schemas.mjs`, `test-uninstall-activation.mjs` | byte-coupling satisfied by the V1–V6 sweep; test files themselves untouched and stay green |
...
node tests/test-uninstall-activation.mjs       # → all pass (1.1.0 registry still accepted under 1.2.0)

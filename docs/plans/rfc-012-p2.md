...
skip only), `.github/workflows/tests.yml`,
...
| 4.12 | — | — | Inspect `git diff --check`, `git diff --stat`, and the full diff against the allowed file set. | diff contains only the 8 listed files; `scripts/lib/promotion-sources.mjs` byte-identical |
...
node scripts/em-workflow-validate.mjs            # → exit 0, "valid":true against a store holding run-records
git diff --name-only                             # → 8 files only (the 6 pre-review + schemas/runtime/learning-io.schema.json + scripts/em-workflow-validate.mjs); excludes scripts/lib/promotion-sources.mjs
git diff --check                                 # → clean
```

...
**Version literals (all six byte-coupled anchors move together — §19.3c coupling
map, scout-verified 2026-07-19 on `bd276b0`):**
...
**S3 scout fold decisions (2026-07-19).** (1) The MINOR bump is SIX anchors, not
...
`typed_versioned/forward` assertion; it moves to `"1.3.0"`. (2) The gauntlet
...
stay declared (labels flip only at full S3+S4 graduation, §3).
...
`tests/test-plugin-registry.mjs`, `tests/test-p0-schemas.mjs`,
...
```bash
node tests/test-em-promote.mjs                 # → 28 pass, 0 fail, 1 skipped (gauntlet::testFingerprintRevalidation names REQ-16/S4)
...
git diff --name-only                            # → subset of the 12-file allowlist; excludes em-promote.mjs and the workflow

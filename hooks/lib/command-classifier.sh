...
_classify_segment() {
  ...
  case "$first" in
    ...
    tee|cp|mv|dd)
      ...
      # the non-marker content-write surface.) Placed AFTER Tier 0 so an explicit
      # `read_only` override (the deny-hint escape) still wins for a genuinely
      # benign use — symmetric with the redirect reasons. No write-target
      # discrimination (the classifier surfaces none), so an external-target
      # write over-arms conservatively, same as redirects; the deny-hint is the
      # escape. (`dd` reading to stdout, `cp`/`mv` outside the repo over-arm —
      # accepted, escapable. `rm` deletion stays a separate non-content class.)
      printf '%s\t\t%s\n' "shared_write" "cmd_content_write"
      return 0
      ;;

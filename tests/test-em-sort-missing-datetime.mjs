...
// Seed one well-formed episode via the REAL em-store, then append two
// malformed index rows the way a foreign harness did (raw JSONL append,
// bypassing em-store):
...
test('em-list exits 0 and returns all rows when date/time are missing or numeric', () => {
  const s = makeSandbox()
  try {
    const goodId = seedStore(s)
    const r = run(LIST, ['--project', 'sort-dt-test'], s)
    ...
    assert.ok(ids.includes(MISSING_ID), `no-date/time row missing from ${ids}`)
    assert.ok(ids.includes(NUMERIC_ID), `numeric-date row missing from ${ids}`)
  } finally { s.cleanup() }
})

test('em-list sorts the well-formed episode ahead of undefined-key rows', () => {
  const s = makeSandbox()
  try {
    const goodId = seedStore(s)
    const r = run(LIST, ['--project', 'sort-dt-test'], s)
    assert.strictEqual(r.code, 0, `em-list crashed: ${r.stderr}`)
    const ids = r.json.episodes.map(e => e.id)
    assert.ok(ids.indexOf(goodId) < ids.indexOf(MISSING_ID),
      `expected ${goodId} before ${MISSING_ID}, got ${ids}`)
  } finally { s.cleanup() }
})

console.log('em-search on malformed index rows:')

test('em-search --no-score exits 0 and returns all rows', () => {
  const s = makeSandbox()
  try {
    const goodId = seedStore(s)
    const r = run(SEARCH, ['--project', 'sort-dt-test', '--no-score', '--no-track'], s)
